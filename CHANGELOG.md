...
### 🔧 功能改进
- **MCP工具连接优化**：改进MCP工具调用时的连接状态检查，当处于非连接状态时自动重新连接，提升工具调用稳定性
- **多语言支持完善**：修复多语言key的处理逻辑，改进国际化支持
- **代码风格配置修复**：修复代码风格初始化数据与展示不一致的问题，确保配置的准确性
- **用户中心API增强**：新增API状态过期检测功能，并完成UI走查还原工作
- **插件融合优化**：处理插件融合过程中的差异化问题，提升兼容性

### 🐛 问题修复
- **模型关键字兜底**：增加端侧模型关键字兜底机制，替换特定模型的返回结果ID，提升响应准确性
- **构建流程优化**：自动增量构建镜像并回写镜像标签到Helm Chat Values，改进部署流程
- **终端兼容性增强**：修复在某些环境下 `hasColors` 方法未定义导致的运行时错误，使用可选链调用提升稳定性
- **组件性能优化**：优化 `compatibleChalk` 初始化时机，避免不必要的提前创建，提升组件渲染性能

### 📖 日志完善
- **日志补充**：补充关键操作的日志记录，便于问题排查和系统监控
...
- **终端快捷键配置**：新增 `/terminal-setup` 命令，支持自动配置 Shift+Enter 换行快捷键绑定
